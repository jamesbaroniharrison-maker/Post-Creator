...
Usage (from the linkedin_content_engine/ app directory, with the venv active):
    python -m linkedin_content_engine.capture.run --file "path/to/voice_note.wav"
    python -m linkedin_content_engine.capture.run --file "path/to/photo.jpg"
    python -m linkedin_content_engine.capture.run --text "Quick note about today"
"""

import argparse

from linkedin_content_engine.capture.ingest import ingest_file, ingest_text
